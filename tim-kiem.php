...
// Kiểm tra xem biến `s` có được gửi từ form hay không
if (isset($_GET['s'])) {
    // Lấy từ khóa tìm kiếm từ biến `s`
    $keyword = $_GET['s'];

    // Lấy danh sách sản phẩm có tên chứa từ khóa tìm kiếm
    $query = "SELECT * FROM sanpham WHERE ten_san_pham LIKE '%$keyword%' ";
    $result = mysqli_query($conn, $query);

    if (mysqli_num_rows($result) > 0) {
        // Hiển thị số kết quả tìm kiếm
        $numResults = mysqli_num_rows($result);
        echo '<h3>Tìm thấy ' . $numResults . ' kết quả phù hợp với từ khóa "' . $keyword . '"</h3>';
        ...
        echo '<h3>Không tìm thấy kết quả phù hợp với từ khóa "' . $keyword . '"</h3>';
    }
}
